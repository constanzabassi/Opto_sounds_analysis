function [output,output_relative] = intersect_sig_cells(sig_mod_boot_thr,sig_mod_boot_thr2, mod_indexm,varargin)
output = {};
output_relative = [];
...
if nargin < 4
    for m = 1:length(sig_mod_boot_thr)
        output{m} = intersect(sig_mod_boot_thr{1,m},sig_mod_boot_thr2{1,m});
        output_relative = [output_relative, reshape(output{m}, 1, [])+numcells(m)];
    end
else
