    ...
    for context = 1:size(deconv_response,1) 
        ...
        for mouse = 1:size(deconv_response,2)
            ...
            if ~isempty(deconv_response{1,mouse,1}.stim) && size(deconv_response{context,mouse,celtype}.stim,2) > 1
                ...
                for trial = 1:length(stim_trials{2,mouse}{2,context});%1:size(deconv_response{context,mouse,celtype}.stim,1)
                    t = stim_trials{2,mouse}{2,context}(trial);
                ...
                if ~isempty(deconv_response{1,mouse,1}.ctrl) && size(deconv_response{context,mouse,celtype}.ctrl,2) > 1
                    for trial = 1:length(ctrl_trials{2,mouse}{2,context});% 1:size(deconv_response{context,mouse,celtype}.ctrl,1)
                        t = ctrl_trials{2,mouse}{2,context}(trial);
