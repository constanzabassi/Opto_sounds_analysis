function [mod_index_by_pool,selectivity_results_all] = analyze_selectivity_pools(selectivity_indexm, mod_indexm, mod_index_results, sig_mod_boot_thr, params)

    % Initialize results structure for all datasets
    mod_index_by_pool = cell(1, length(selectivity_indexm));
    ...
    for dataset_index = 1:length(selectivity_indexm)
        ...
        mod_index_by_pool{dataset_index} = analyze_mod_by_selectivity_pool(...
    ...
    selectivity_results_all = combine_dataset_results(mod_index_by_pool, total_cells_per_dataset);
