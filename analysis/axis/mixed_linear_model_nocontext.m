function [lme,tbl,proj_all,engagement_proj_all,context_all] = mixed_linear_model_nocontext(proj, axis_type,celltype,frame_range_pre,frame_range_post,contexts,varargin)
%% linear mixed models 
...
for dataset = 1:n_datasets
    for ctx = contexts   % context 1=active, 2=passive
        ...
        if length(celltype) > 1
            if nargin > 6 %compare to something besides context
        ...
        if nargin > 7 %change reference to get intercept and slopes for each context
