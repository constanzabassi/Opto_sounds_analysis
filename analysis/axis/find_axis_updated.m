function [proj,proj_ctrl,proj_norm,proj_ctrl_norm, weights,trial_corr_context,percent_correct,real_activity_all,real_activity_all_ctrl,real_activity_all_norm,percent_correct_concat,proj_concat,proj_concat_norm] = find_axis_updated(dff_st, chosen_mice, all_celltypes,sig_mod_boot,varargin)
        total_trials = {};
        ...
        for current_dataset = chosen_mice
            ...
            [stim_splits_ds, ctrl_splits_ds, ~, ~] = make_threeway_cv_splits(total_trials_stim, total_trials_ctrl);
            for ctx = 1:2
   ...
   for split = 1:3
       split
        ...
        for current_dataset = chosen_mice
            ...
            trainB_stim_all = [stim_splits{current_dataset,1}(split).trainB,stim_splits{current_dataset,2}(split).trainB];
            ...
            trainB_ctrl_all = [ctrl_splits{current_dataset,1}(split).trainB,ctrl_splits{current_dataset,2}(split).trainB];
    ...
    %             [trials_ctrl] = size(ctrl_matrix, 1);
                ...
                nanmean(ctrl_matrix(ctrl_splits{current_dataset,1}(split).trainB, :, bframes), [1, 3]); % active sound total_trials{current_dataset, 1, 2}
                nanmean(ctrl_matrix(ctrl_splits{current_dataset,2}(split).trainB, :, bframes), [1, 3])]; % passive sound total_trials{current_dataset, 2, 2}
                ...
                nanmean(stim_matrix(stim_splits{current_dataset,1}(split).trainB, :, bframes), [1, 3]); % active sound total_trials{current_dataset, 1, 2}
                nanmean(stim_matrix(stim_splits{current_dataset,2}(split).trainB, :, bframes), [1, 3])]; % passive sound total_trials{current_dataset, 2, 2}
                ...
                stim_trial_neurons = squeeze(mean(stim_matrix(trainB_stim_all,:,aframes),3)); %mean across time
                ctrl_trial_neurons = squeeze(mean(ctrl_matrix(trainB_ctrl_all,:,aframes),3)); %mean across time
                ...
                ctrl_train_mean = squeeze(mean(ctrl_matrix(trainB_ctrl_all, :, :), 1));  % [neurons x time]
                stim_train_mean = squeeze(mean(stim_matrix(trainB_stim_all, :, :), 1));  % [neurons x time]
