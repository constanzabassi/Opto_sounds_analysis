function norm_aligned_data = normalize_aligned_data_2d(aligned_data,method,smoothing_param)
%%% INPUTS: matrix size trials x time/frames
...
trial_times = 1:size(aligned_data,2):length(temp_aligned);
%put back into nice trial x neuron x time matrix
norm_aligned_data = [];
for t = 1:size(aligned_data,1)  
    norm_aligned_data(t,:) = norm_temp(trial_times(t):trial_times(t)+size(aligned_data,2)-1);
end
