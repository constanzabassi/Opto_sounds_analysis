...
%     mod_type           - String specifying the modulation index type, e.g. 'ctrl', 'influence', or 'prepost'.
...
for current_dataset = 1:length(info.mouse_date)
    ...
    for context = 1:3  % Assuming context 1: active, context 2: passive, context 3: spontaneous.
        % Get condition labels from trial info 
        if context == 1
            current_conditions = [all_trial_info(current_dataset).opto.condition];
            current_conditions_ctrl = [all_trial_info(current_dataset).ctrl.condition];
        elseif context == 2
            current_conditions = [all_trial_info_passive(current_dataset).opto.condition];
            current_conditions_ctrl = [all_trial_info_passive(current_dataset).ctrl.condition];
