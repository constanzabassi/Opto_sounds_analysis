...
nContexts = 3;
if strcmpi(mod_type,'prepost_sound') || strcmpi(mode,'selectivity') || strcmpi(mod_type,'prepost_sound_num')
    nContexts = 2;
end
...
for current_dataset = 1: length(info.mouse_date)
    ...
    for context = 1:nContexts  % Assuming context 1: active, context 2: passive, context 3: spontaneous.
        ...
        if context == 1
            if strcmpi(mod_type,'prepost_sound') || strcmpi(mode,'selectivity') || strcmpi(mod_type,'prepost_sound_num') %for sound alignment I included [control, sound_only trials] so I need to concatenate trial types here)
