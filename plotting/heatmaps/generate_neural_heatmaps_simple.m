...
if isfield(params, 'savepath')
    ...
    save_fname = sprintf('heatmap_stim_%s_%s_%s.png', type, save_name, ctx_str);

    saveas(gcf, fullfile(params.savepath, save_fname));

    save_fname = sprintf('heatmap_stim_%s_%s_%s.pdf', type, save_name, ctx_str);
    exportgraphics(gcf,fullfile(params.savepath, save_fname), 'ContentType', 'vector','BackgroundColor', 'none');
end

figure(2);clf;
%PLOT BASIC HEATMAPS (CTRL)
for context = 1:nContexts
    % Plot using consistent sorting
    subplot(1,nContexts,context)
...
if isfield(params, 'savepath')
        % Define the filename
    ctx_str = sprintf('%d', context_to_plot);
    save_fname = sprintf('heatmap_ctrl_%s_%s_%s.png', type, save_name, ctx_str);

    saveas(gcf, fullfile(params.savepath, save_fname));

    save_fname = sprintf('heatmap_ctrl_%s_%s_%s.pdf', type, save_name, ctx_str);
    exportgraphics(gcf,fullfile(params.savepath, save_fname), 'ContentType', 'vector','BackgroundColor', 'none');
end
