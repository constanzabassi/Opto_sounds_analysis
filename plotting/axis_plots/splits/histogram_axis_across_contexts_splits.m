function [hist_stats,all_trial_means_contexts] =  histogram_axis_across_contexts_splits(chosen_datasets,proj, axis_type, celltype, bin_edges,frame_range,colorss,legend_string,save_dir)
...
for ctx = 1:2
    ...
    for dataset = chosen_datasets
        ...
        for splits = 1:n_splits
            data = proj{splits,dataset, celltype, ctx}.(lower(axis_type));
            trial_means = mean(data(:, frame_range), 2); % mean per trial in specified frames
            temp = [temp;trial_means];
        end
        trial_means_across_splits = temp; %includes trial means across all splits
        all_trial_means = [all_trial_means; trial_means_across_splits];
        all_trial_means_datasets(ctx,dataset) = mean(trial_means_across_splits);
        dataset_means(dataset) = mean(trial_means_across_splits); % <- compute dataset-level average
    end
...
if ~isempty(save_dir)
    mkdir(save_dir)
    cd(save_dir)
    saveas(800,strcat('hist_splits_axis_',num2str(axis_type),'_n_',num2str(length(chosen_datasets)),'.svg'));
    saveas(800,strcat('hist_splits_axis_',num2str(axis_type),'_n_',num2str(length(chosen_datasets)),'.fig'));
    exportgraphics(figure(800),strcat('hist_splits_axis_',num2str(axis_type),'_n_',num2str(length(chosen_datasets)),'.pdf'), 'ContentType', 'vector');
    save(strcat('hist_splits_axis_',num2str(axis_type),'stats_n',num2str(length(chosen_datasets))),'hist_stats');
end
