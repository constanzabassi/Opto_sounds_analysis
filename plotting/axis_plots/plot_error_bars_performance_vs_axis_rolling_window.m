function [binned_perf_all,errorbar_stats] = plot_error_bars_performance_vs_axis_rolling_window(chosen_datasets,proj, axis_type, celltype,percent_correct,frame_range,edges_values,num_bins,save_dir)
...
%         bin_idx = trial_means >= edges(b) & trial_means < edges(b+1);
    for b = 1:num_bins
        bin_start = bins(b);
        bin_idx = trial_means >= bin_start & trial_means < bin_start + window;
...
if ~isempty(save_dir)
    mkdir(save_dir)
    cd(save_dir)
    saveas(8022,strcat('errorbar_rollingwin_correct_vs_axis_',num2str(axis_type),'_n_',num2str(length(chosen_datasets)),'_edges_',num2str(edges_values),'.svg'));
    saveas(8022,strcat('errorbar_rollingwin_correct_vs_axis_',num2str(axis_type),'_n_',num2str(length(chosen_datasets)),'.fig'));
    exportgraphics(figure(8022),strcat('errorbar_rollingwin_correct_vs_axis_',num2str(axis_type),'_n_',num2str(length(chosen_datasets)),'_edges_',num2str(edges_values),'.pdf'), 'ContentType', 'vector');

    saveas(804,strcat('heatmap_rollingwin_correct_vs_axis_',num2str(axis_type),'_n_',num2str(length(chosen_datasets)),'_edges_',num2str(edges_values),'.fig'));
    exportgraphics(figure(804),strcat('heatmap_rollingwin_correct_vs_axis_',num2str(axis_type),'_n_',num2str(length(chosen_datasets)),'_edges_',num2str(edges_values),'.pdf'), 'ContentType', 'vector');

    save(strcat('errorbar_rollingwin_stats_n',num2str(length(chosen_datasets)),'_edges_',num2str(edges_values)),'errorbar_stats');
