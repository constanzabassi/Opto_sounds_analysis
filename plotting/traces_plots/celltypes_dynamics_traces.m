function mean_across_celltype = celltypes_dynamics_traces(deconv_response,colors,lineStyles_contexts,celltypes_ids,frames,stim_frame,save_dir,thrs,ylims)
f1 = figure(96);clf
...
if ~isempty(save_dir)
    mkdir([save_dir '\dynamics'])
    cd([save_dir '\dynamics'])
    saveas(96,strcat('avg_dynamics_traces',(thrs),'.svg'));
    saveas(96,strcat('avg_dynamics_traces',(thrs),'.fig'));
end
