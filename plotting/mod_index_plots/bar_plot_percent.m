...
%     nexttile
    ...
    for ct = 1:num_percents
        mean_all(ct,ce) = squeeze(mean(mean_percent(ct,:,ce),2,'omitnan'));
        std_all = std(mean_all(ct,ce), 0, 2);
        stats{ct,ce} = get_basic_stats(squeeze(mean_percent(ct,:,ce)));

        bar_context = [bar_context; mean_all(ct,ce)];
        SEM = std(squeeze(mean_percent(ct,:,ce)),'omitnan')/sqrt(length(mean_percent(ct,:,ce)));
        SEM_cells = [SEM_cells; SEM];
        all_stats.stats(ct,ce) = get_basic_stats(squeeze(mean_percent(ct,:,ce)));
     ...
     hold on
        ...
        xticks([1:length(celltype_names)])
        xticklabels([celltype_names])


%     [KW_Test.context_p_val,KW_Test.context_tbl, KW_Test.context_stats_cell] = kruskalwallis(squeeze(mean_percent(:,:,ce))',[1:length(celltype_names)],'off');
%     possible_tests = nchoosek(1:length(celltype_names),2);
%         yl = ylim;    
%         cct = 0;
...
%     
    ...
    % Customize the plot
    set(gca, 'XTickLabel', xlabels{ct},'XTickLabelRotation',45);
    if ce == 1
        ylabel(string);
    end
    utils.set_current_fig;
    xlim([0 length(num_percents)+.99])
...
%     saveas(gcf,strcat(['bar_' string '_contexts.svg']));
    exportgraphics(gcf,strcat('bar_percents', strjoin(cellstr(celltype_names), ''), '_contexts.pdf'), 'ContentType', 'vector');
    saveas(gcf,strcat('bar_percents', strjoin(cellstr(celltype_names), ''), '_contexts.fig'));
