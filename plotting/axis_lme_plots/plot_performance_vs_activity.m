function plot_performance_vs_activity(percent_correct_concat,neural_activity,window_bins,save_dir,edge_input,varargin)
%% collect correctness across mice
correct_all_ctrl = []; animal_id_all=[]; temp = [];
for dataset = 1:24
    ...
    temp = [temp,neural_activity{dataset}];
end

neural_activity_new = temp ;
...
for a = 1:length(animal_ids)
    ...
    engagement = neural_activity_new(idx);    % Continuous engagement projection
...
xlabel({'Mean Activity Pre';'(z-scored)'});
...
ylabel({'Mean Activity Pre';'(z-scored)'});
...
if ~isempty(save_dir)
    mkdir(save_dir)
    cd(save_dir)
    saveas(404,strcat('errorbar_performance_vs_activity_axis_windownbin',num2str(window_bins),'edges',num2str(edges(1)),num2str(edges(end)),'.fig'));
    exportgraphics(figure(404),strcat('errorbar_performance_vs_activity_windownbin',num2str(window_bins),'edges',num2str(edges(1)),num2str(edges(end)),'.pdf'), 'ContentType', 'vector');
    exportgraphics(figure(405),strcat('scatter_performance_vs_activity_windownbin',num2str(window_bins),'edges',num2str(edges(1)),num2str(edges(end)),'.pdf'), 'ContentType', 'vector');
